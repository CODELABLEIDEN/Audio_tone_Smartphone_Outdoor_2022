...
% Oirignal spectral analysis used mean(abs) [ sqrt(mean|x|)] the code was changed to use
% mean(abs).^2 [ sqrt(mean|x|).^2]. 

%% Output dir 
% for aperiodic signal 
dir_foofa = '\Smartphone_AEP_2021\AllSubjectscwt\fooof_rerun\A'; 
    ...
    if exist([dirRW(d).folder, '\',dirRW(d).name, '\RegOutdoor']) == 7
        ...
        for f = 1:size(Yr,3)
            ...
            parfor ch = 1:64
                ...
                settings = struct();
                fooof_results = fooof(LIMO.Freq, squeeze(Yr(ch,:,f)).^4, [0.1 40],settings, true); % extra ^2 to return to non squared scale as there were one too many squares. 
        ...
        %% trim data 
        if ff> 0
            % Adjust RejIdx to be less than the max Yr_ dimension
            RejIdx(RejIdx>f) = []; 
            % Rejections
            Yr_s(:,RejIdx) = [];
            Yr_e(:,RejIdx) = [];
