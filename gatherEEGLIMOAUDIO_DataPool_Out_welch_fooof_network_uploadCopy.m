...
dir_foofa = '\AllSubjectswelch\fooof\A'; 

%% General limo 
load('\Clean_Codes\EEG\Human_electrodes\expected_chanlocs')
load('\Clean_Codes\EEG\Human_electrodes\channeighbstructmat')


%% input dir 

dir_cwt = '\Smartphone_AEP_2021\AllSubjectswelch'; 

dirRW = dir([dir_cwt, '\RW*']);

for d = 22:length(dirRW)
    ...
    if exist([dirRW(d).folder, '\',dirRW(d).name, '\RegOutdoor']) == 7
        ...
        for f = 1:size(Yr,3)
            ...
            parfor ch = 1:64
                ...
                settings = struct();
                fooof_results = fooof(LIMO.Freq, squeeze(Yr(ch,:,f)), [0.1 40],settings, true);
